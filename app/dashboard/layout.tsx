...
function DashboardLayoutContent({ children }: DashboardLayoutProps) {
  ...
  const { primaryColor, secondaryColor, companyLogo, companyName, isLoading } =
    useTheme()
        ...
        if (companiesRes.ok) {
          ...
          if (activeCompany) {
            const transRes = await fetch(
              `/api/accounting/transactions?companyId=${activeCompany.id}`,
            )
            if (transRes.ok) {
  ...
  const handleTransactionSearch = (
    ...
    if (e.key === "Enter" && transactionSearch.trim()) {
      const found = transactions.find(
        (t) =>
          t.code.toLowerCase() === transactionSearch.toLowerCase() ||
  ...
  const filteredTransactions = transactionSearch.trim()
    ? transactions
        .filter(
          (t) =>
            t.code.toLowerCase().includes(transactionSearch.toLowerCase()) ||
            t.name.toLowerCase().includes(transactionSearch.toLowerCase()),
        )
        .slice(0, 5)
    : []
      ...
      icon: (
        ...
        >
          <path
            ...
            strokeLinejoin="round"
            strokeWidth={2}
            d="M8 7h.01M12 7h.01M16 7h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
          />
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M9 12l2-2 2 2m-2-2v6m7-3a7 7 0 11-14 0 7 7 0 0114 0z"
          />
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            strokeWidth={2}
            d="M3 12h3m15 0h-3M9 20h6M7 16h10a1 1 0 001-1v-2a1 1 0 00-1-1H7a1 1 0 00-1 1v2a1 1 0 001 1z"
          />
        </svg>
      ),
  ...
  return (
    <div className="flex h-screen bg-gray-50">
            ...
            if (item.href === "/dashboard") {
              ...
              isActive =
                ...
                (pathname?.startsWith(item.href + "/") &&
                  !pathname?.startsWith("/dashboard/accounting/entries"))
            } else {
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="bg-white shadow-sm border-b border-gray-200 px-6 py-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-gray-800">
              <Link href="/dashboard">Echo ERP</Link>
            </h1>
        ...
        {/* Page Content */}
        <main
          className={`flex-1 overflow-y-auto ${pathname === "/dashboard/chat" ? "" : "p-6"}`}
        >
          {children}
        </main>
      </div>
