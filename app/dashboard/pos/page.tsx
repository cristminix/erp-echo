...
export default function POSPage() {
  ...
  const fetchData = async () => {
    try {
      // Mendapatkan perusahaan aktif
      const companiesRes = await fetch("/api/companies")
      if (companiesRes.ok) {
        const companies = await companiesRes.json()
        const activeCompany =
          companies.find((c: any) => c.active) || companies[0]
        if (activeCompany) {
          setCompanyId(activeCompany.id)

          // Mendapatkan produk
          const productsRes = await fetch(
            `/api/products?companyId=${activeCompany.id}`,
          )
          if (productsRes.ok) {
          ...
          const contactsRes = await fetch(
            `/api/contacts?companyId=${activeCompany.id}`,
          )
          if (contactsRes.ok) {
  ...
  const addToCart = (product: Product) => {
    ...
    if (existingItemIndex >= 0) {
      ...
      console.log("Menambahkan item baru:", newItem)
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setCart([])
        setQuantityInput("1")
        toast.success("✓ Penjualan berhasil diselesaikan", {
          duration: 3000,
          style: {
            background: "#10b981",
            color: "#fff",
        ...
        console.error("Kesalahan server:", errorData)
        toast.error(`Error: ${errorData.error || "Gagal membuat faktur"}`, {
          duration: 4000,
        })
      }
    } catch (error) {
      console.error("Error membuat faktur:", error)
      toast.error(
                ...
                >
                  <div className="text-left">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span
                        className={`text-xs ${product.stock <= 5 ? "text-red-500" : "text-gray-500"}`}
                      >
                        Stock: {product.stock}
                      </span>
                    </div>
                      ...
                      <div className="text-right">
                        ...
                        {item.tax > 0 && (
                          <div className="text-xs text-gray-500">
                            PPN: Rp{item.taxAmount.toFixed(2)}
                          </div>
                        )}
                      </div>
                    </div>
          ...
          <div className="p-4 bg-white border-t border-gray-200">
            <div className="space-y-2 mb-4">
              <div className="flex justify-between text-gray-600">
                <span>Subtotal:</span>
                <span>Rp{totals.subtotal.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-gray-600">
                <span>PPN:</span>
                <span>Rp{totals.taxAmount.toFixed(2)}</span>
              </div>
              <div className="flex justify-between text-2xl font-bold text-gray-800 pt-2 border-t border-gray-200">
                <span>Total:</span>
                <span>Rp{totals.total.toFixed(2)}</span>
              </div>
            </div>
