...
async function odooRpcCall(url: string, db: string, uid: number, password: string, model: string, method: string, params: any[] = []) {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const products = searchReadData.result.map((item: any) => ({
      ...
      code: item.default_code || `PROD-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
    ...
    console.log(`✅ Total productos importados: ${importedCount} de ${products.length}`);
    if (errors.length > 0) {
