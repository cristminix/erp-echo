...
export default function AttendanceMobilePage() {
  ...
  const fetchProjects = async () => {
    try {
      const res = await fetch(
        `/api/projects?companyId=${data?.company.id}&public=true`,
      )
      if (res.ok) {
  ...
  const hasOpenAttendance =
    data.todayAttendance && !data.todayAttendance.checkOut
    ...
    >
      <div className="bg-white rounded-3xl shadow-2xl p-6 sm:p-8 max-w-md w-full">
        ...
        {data.todayAttendance && (
          <div
            className={`${hasOpenAttendance ? "bg-blue-50 border-blue-200" : "bg-green-50 border-green-200"} border-2 rounded-2xl p-4 mb-6`}
          >
              ...
              <div className="flex-1">
                <div className="space-y-1 text-sm">
                  <p className="text-gray-600">
                    ...
                    {new Date(data.todayAttendance.checkIn).toLocaleTimeString(
                      "es-ES",
                      {
                        hour: "2-digit",
                        minute: "2-digit",
