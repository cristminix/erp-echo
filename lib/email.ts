...
export async function sendVerificationEmail(
  ...
  try {
    ...
    const html = `
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <p>Hola ${name},</p>
            <p>Gracias por registrarte en Echo ERP. Para completar tu registro, por favor ingresa el siguiente código de verificación:</p>
    ...
    await transporter.sendMail({
      from: `"Echo ERP" <${smtpFrom}>`,
      to,
      subject: "Verifica tu email - Echo ERP",
      html,
    })
...
export async function sendEmail({
  ...
  try {
    ...
    if (!company) {
      throw new Error("Perusahaan no encontrada")
    }
    ...
    if (
      ...
      throw new Error(
        "Configuración SMTP incompleta. Por favor configure el SMTP en la empresa.",
      )
    }
...
function replaceTemplateVariables(
  ...
  result = result.replace(
    /\{\{company\.primaryColor\}\}/g,
    company.primaryColor || "#10b981",
  )
  result = result.replace(
    /\{\{company\.secondaryColor\}\}/g,
    company.secondaryColor || "#059669",
  )
